...
export const SITE_DESC =
  "Catálogo chico y confiable de APIs LatAm pagables por uso vía x402, con probing en vivo y taxonomía regional.";
...
export function mcpManifestDocument(origin, { staticHost = false } = {}) {
  ...
  return {
    name: `${SITE_NAME} MCP`,
    description:
      "Herramientas MCP en español para descubrir y llamar APIs x402 LatAm. El call-through en vivo es solo local.",
    version: SITE_VERSION,
    language: "es",
    tools: staticHost
      ? tools.map((tool) => ({
          ...
          how_to_call:
            tool.name === "llamar_servicio"
              ? "No hay MCP pay-through público. Tomá `url`/`payTo`/`endpoints` de /discovery/resources y pagá el seller directo (p. ej. x402-buyer). En local, POST /mcp/llamar_servicio solo devuelve instrucciones."
              : `En el host estático: filtrá el catálogo en ${origin}/discovery/resources o ${origin}/api/apis.json. En local: POST ${LOCAL_ORIGIN}/mcp/${tool.name}.`,
        }))
      : tools,
    ...(staticHost
      ? {
